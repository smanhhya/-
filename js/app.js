...
// نص ثابت لقيم النصوص الافتراضية (للرجوع إليها عند الحاجة)
const textsConfig = [
    { id: 'cartTotalLabel', default: 'الإجمالي (بدون التوصيل)' },
    { id: 'viewCartBtn', default: 'عرض السلة' },
    { id: 'checkoutTitle', default: 'سلة المشتريات' },
    { id: 'zoneLabel', default: 'المنطقة' },
    { id: 'nameLabel', default: 'الاسم' },
    { id: 'phoneLabel', default: 'الموبايل' },
    { id: 'addressLabel', default: 'العنوان' },
    { id: 'whatsappCheck', default: 'إرسال تأكيد واتساب' },
    { id: 'promoBtn', default: 'تفعيل' },
    { id: 'subtotalLabel', default: 'قيمة الطلبات:' },
    { id: 'deliveryLabel', default: 'التوصيل' },
    { id: 'finalTotalLabel', default: 'الإجمالي النهائي:' },
    { id: 'menuTitle', default: 'قائمة الأطباق' },
    { id: 'extrasTitle', default: 'منتجات أخرى' }
];

// ==================== دوال السلة (LocalStorage) ====================
function saveCart() {
    try {
        localStorage.setItem('sman_cart', JSON.stringify(cart));
    } catch (e) {
        console.warn('تعذر حفظ السلة');
    }
}
...
function loadCart() {
    try {
        ...
        cart = {};
        localStorage.removeItem('sman_cart');
    }
    updateUI();
}

// حساب المخزون المتاح مع مراعاة الكمية الموجودة في السلة
...
function applySettingsToUI() {
    ...
    if (globalSettings.bannerActive && globalSettings.bannerText && globalSettings.bannerText.trim() !== '') {
        ...
        if (globalSettings.bannerBgColor) banner.style.backgroundColor = globalSettings.bannerBgColor;
        if (globalSettings.bannerTextColor) banner.style.color = globalSettings.bannerTextColor;
        if (globalSettings.bannerFontSize) banner.style.fontSize = globalSettings.bannerFontSize + 'px';

        const iconEl = document.getElementById('top-banner-icon');
        if (iconEl && globalSettings.bannerIcon) {
            iconEl.className = globalSettings.bannerIcon;
        }

        const linkEl = document.getElementById('top-banner-link');
        if (linkEl && globalSettings.bannerLink) {
            linkEl.href = globalSettings.bannerLink;
            linkEl.classList.remove('hidden');
        } else if (linkEl) {
            linkEl.classList.add('hidden');
        }

        if (globalSettings.bannerAnimated) {
    ...
    if (promoContainer) {
        if (globalSettings.showPromoField !== false) {
...
function renderDeliveryZones() {
    ...
    globalDeliveryZones.forEach(zone => {
        ...
        option.textContent = zone.price === 0
            ? `${zone.name} (يحدد لاحقاً)`
            : `${zone.name} (${zone.price} جنيه)`;
        select.appendChild(option);
    });
    if (currentVal && globalDeliveryZones.find(z => z.id === currentVal)) {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // احتياط في حال عدم تحميل البيانات من السحابة
    setTimeout(() => {
...
window.setupEventListeners = function() {
    ...
    if (promoInput) {
        promoInput.addEventListener('input', () => {
            if (appliedPromo && !appliedPromo.isLoyalty && promoInput.value.trim() === '') {
                appliedPromo = null;
        ...
        firebase.auth().onAuthStateChanged(user => {
            if (user) {
                ...
                if (!user.isAnonymous && user.email) {
                    autoOpenAdminIfLoggedIn(user);
                }
            }
        });
    } catch (e) {
        console.warn("تعذر الاتصال بـ Firebase");
    }
}

// الاستماع لتغيرات قاعدة البيانات
function listenToDatabase() {
    ...
    db.collection('inventory').doc('prices').onSnapshot(doc => {
        if (doc.exists) Object.assign(globalPrices, doc.data());
    });

    // الخصومات النشطة
    db.collection('inventory').doc('discounts_status').onSnapshot(doc => {
        if (doc.exists) Object.assign(globalDiscounts, doc.data());
    });

    // الأسعار القديمة
    db.collection('inventory').doc('old_prices').onSnapshot(doc => {
        if (doc.exists) Object.assign(globalOldPrices, doc.data());
    });
    ...
    db.collection('inventory').doc('stats').onSnapshot(doc => {
        if (doc.exists) {
            ...
            if (salesEl) salesEl.innerText = dailyStats.sales || 0;
            if (ordersEl) ordersEl.innerText = dailyStats.orders || 0;
        }
    });
}
...
window.applyPromoCode = function() {
    ...
    let subTotal = 0;
    for (let id in cart) {
        subTotal += cart[id].quantity * (globalPrices[id] || productsInfo[id].basePrice);
    }
    ...
    if (promo.usesLeft !== null && promo.usesLeft !== undefined && promo.usesLeft <= 0) {
        msg.innerText = "عفواً، انتهى الحد الأقصى لاستخدام هذا الكود.";
        msg.className = "text-[11px] font-bold mt-1 text-red-500";
        msg.classList.remove('hidden');
        return;
    }
    if (promo.expiryDate && new Date(promo.expiryDate) < new Date(new Date().toDateString())) {
        msg.innerText = "عفواً، هذا الكود منتهي الصلاحية.";
        msg.className = "text-[11px] font-bold mt-1 text-red-500";
        msg.classList.remove('hidden');
        return;
    }
    if (promo.minOrder && subTotal < promo.minOrder) {
        msg.innerText = `عشان تفعل الكود ده، لازم طلباتك تتخطى ${promo.minOrder} ج.م`;
        msg.className = "text-[11px] font-bold mt-1 text-red-500";
        msg.classList.remove('hidden');
        return;
    }
...
window.getCardActionHTML = function(id) {
    ...
    if (globalSettings.storeOpen === false) {
        return `<div class="w-full bg-gray-100 text-gray-400 font-bold py-2 rounded-xl text-sm border border-gray-200 text-center cursor-not-allowed">${txtClosed}</div>`;
    }
    ...
    if (available === 0 && inCart === 0) {
        return `<div onclick="openVipPreOrder('${id}')" class="w-full bg-gradient-to-r from-brand-yellow to-yellow-500 text-brand-navy hover:from-yellow-500 hover:to-yellow-600 font-black py-2 rounded-xl transition-all shadow-sm text-xs flex justify-center items-center gap-1 cursor-pointer select-none border border-yellow-400 transform hover:scale-105"><i class="fa-solid fa-crown"></i> ${txtVip}</div>`;
    }

    if (inCart > 0) {
        return `<div class="flex items-center justify-between bg-brand-light/50 border border-brand-cyan/40 rounded-xl p-1 h-[40px]">
            <div onclick="updateQuantity('${id}', 1)" class="w-9 h-full bg-white text-brand-cyanDark rounded-lg shadow-sm font-black text-xl flex items-center justify-center cursor-pointer select-none">+</div>
            <span class="font-black text-brand-navy text-lg min-w-[28px] text-center select-none">${inCart}</span>
            <div onclick="updateQuantity('${id}', -1)" class="w-9 h-full bg-white text-red-500 rounded-lg shadow-sm font-black text-2xl flex items-center justify-center cursor-pointer select-none pb-1">-</div>
        </div>`;
    }
...
window.sendVipWhatsApp = function(itemName) {
    ...
    let template = globalSettings.vipWhatsappTemplate ||
...
window.renderProducts = function() {
    ...
    Object.keys(productsInfo).forEach(id => {
        ...
        if (isDiscountActive) {
            ...
            priceHtml = `<span class="font-black text-brand-cyanDark text-lg"><span id="price-display-${id}">${currentPrice}</span> <span class="text-[10px] text-gray-400">ج.م</span></span>`;
        }

        let bestSellerHtml = isBestSeller
            ? `<div class="absolute bottom-2 right-2 bg-brand-navy text-brand-yellow text-[10px] font-black px-2 py-1 rounded shadow z-10 border border-brand-yellow/30">الأكثر طلباً 🔥</div>`
            : '';
        ...
        if (!item.isExtra) {
            mainProductsCount++;
            container.innerHTML += `<div class="bg-white rounded-2xl shadow-sm border ${isDiscountActive ? 'border-red-200' : 'border-gray-200'} overflow-hidden flex flex-row h-[130px] relative">
                ${saleBadgeHtml}
                <div class="p-3 flex-1 flex flex-col justify-between bg-white min-w-0">
                    ...
                    <div class="flex items-center justify-between mt-auto">
                        ...
                        <div class="w-28 shrink-0" id="card-action-${id}">${window.getCardActionHTML(id)}</div>
                    </div>
                </div>
                <div class="w-32 shrink-0 relative bg-gray-100 border-r border-gray-100">
                    ${bestSellerHtml}
                    <img loading="lazy" src="${imgSrc}" onerror="this.src='data:image/svg+xml;utf8,<svg xmlns=\"http://www.w3.org/2000/svg\"><rect width=\"100%\" height=\"100%\" fill=\"%23f1f5f9\"/></svg>'" class="w-full h-full object-cover">
                </div>
            </div>`;
        } else {
            let extraPriceHtml = isDiscountActive
                ? `<div class="flex gap-2 items-center"><span class="text-red-600 font-black text-sm"><span id="price-display-${id}">${currentPrice}</span> ج</span><span class="text-[10px] text-gray-400 line-through">${oldPrice}</span></div>`
                : `<div class="text-brand-cyanDark font-black text-sm"><span id="price-display-${id}">${currentPrice}</span> ج</div>`;

            extrasContainer.innerHTML += `<div class="bg-white rounded-2xl shadow-sm border ${isDiscountActive ? 'border-red-300' : 'border-gray-200'} overflow-hidden flex flex-row items-center p-2.5 gap-3 h-[90px] relative">
                ${isDiscountActive ? `<div class="absolute top-1 right-1 bg-red-500 w-2 h-2 rounded-full sale-badge"></div>` : ''}
                <div class="flex-1 min-w-0">
                    <div class="flex gap-1 items-center">
                        <h4 class="font-black text-brand-navy text-xs sm:text-sm truncate">${item.name}</h4>
                        ${customTagHtml}
                    </div>
                    ${extraPriceHtml}
                    <div class="text-[10px] font-bold px-1.5 py-0.5 rounded inline-flex border ${stockBadgeClass} shrink-0 mt-1">المتاح: <span id="stock-display-${id}">${available}</span></div>
                </div>
                <div class="w-28 shrink-0" id="card-action-${id}">${window.getCardActionHTML(id)}</div>
                <div class="w-16 h-16 sm:w-20 sm:h-20 shrink-0 relative overflow-hidden rounded-xl border border-gray-100">
                    ${bestSellerHtml}
                    <img loading="lazy" src="${imgSrc}" onerror="this.src='data:image/svg+xml;utf8,<svg xmlns=\"http://www.w3.org/2000/svg\"><rect width=\"100%\" height=\"100%\" fill=\"%23f8fafc\"/></svg>'" class="w-full h-full object-cover">
                </div>
            </div>`;
        }
    });

    if (mainProductsCount === 0) {
        container.innerHTML = `<div class="text-center py-10 text-gray-400 font-bold">${globalSettings.uiTexts?.emptyMenuMsg || "لا توجد منتجات حالياً"}</div>`;
    }
};
...
window.addToCart = function(id) {
    ...
    if (getAvailableStock(id) <= 0) {
        showAlert("عذراً", "الكمية المتاحة لا تكفي حالياً.");
        return;
    }
    if (cart[id]) {
        cart[id].quantity++;
        ...
        cart[id] = {
            quantity: 1,
            price: globalPrices[id] || productsInfo[id].basePrice,
...
window.updateQuantity = function(id, delta) {
    ...
    if (delta === 1) {
        if (getAvailableStock(id) > 0) {
            ...
            showAlert("عذراً", "لا يوجد مخزون إضافي متاح.");
        }
    } else if (delta === -1) {
        cart[id].quantity--;
...
// ==================== تحديث واجهة المستخدم (السلة والأسعار) ====================
window.updateUI = function() {
    ...
    for (let id in cart) {
        ...
        if (cartItemsContainer) {
            cartItemsContainer.innerHTML += `<div class="bg-white p-3 rounded-xl border border-gray-100 flex justify-between items-center shadow-sm">
                <div class="flex-1">
                ...
                <div class="flex items-center gap-2 bg-gray-50 rounded-lg p-1 border border-gray-200 h-[36px]">
                    ...
                    <div onclick="updateQuantity('${id}', -1)" class="w-8 h-full bg-white text-red-500 rounded shadow-sm font-black text-xl flex items-center justify-center pb-1 cursor-pointer select-none">-</div>
                </div>
            </div>`;
        }
    }
    ...
    if (bottomBar) {
        ...
        if (totalItems > 0) {
            ...
            if (cartItemsContainer) {
                cartItemsContainer.innerHTML = `<div class="flex flex-col items-center justify-center py-10 text-gray-400 gap-3">
                    <i class="fa-solid fa-basket-shopping text-5xl text-gray-200"></i>
                    <p class="font-bold text-sm">${globalSettings.uiTexts?.emptyCartMsg || "السلة فارغة"}</p>
                </div>`;
            }
        }
    }

    // متتبع التوصيل المجاني
    ...
    if (appliedPromo && totalItems > 0) {
        if (appliedPromo.minOrder && subTotalPrice < appliedPromo.minOrder) {
            ...
            const msg = document.getElementById('promo-message');
            if (msg) {
                msg.innerText = "تم إلغاء الخصم لأن الطلبات قلت عن الحد الأدنى للكود.";
                msg.className = "text-[11px] font-bold mt-1 text-red-500";
                msg.classList.remove('hidden');
            }
            const promoInput = document.getElementById('promo-code-input');
            ...
            if (appliedPromo.type === 'free_delivery') {
                ...
                discountAmount = appliedPromo.type === 'percent'
                    ? (subTotalPrice * (appliedPromo.discount / 100))
    ...
    if (document.getElementById('cart-delivery-fee')) {
        if (freeDelivery) {
            document.getElementById('cart-delivery-fee').innerHTML = `<span class="text-green-600 font-black">مجاني 🎉</span>`;
        } else {
            document.getElementById('cart-delivery-fee').innerText = (selectedZone && selectedZone.price === 0) ? "يحدد لاحقاً" : `${finalDelivery} ج.م`;
        }
    }
    ...
    if (proceedBtn) {
        if (totalItems > 0 && meetsMinOrder) {
            proceedBtn.disabled = false;
            if (checkoutHintStep1) checkoutHintStep1.classList.add('hidden');
        } else {
            proceedBtn.disabled = true;
            if (totalItems > 0 && !meetsMinOrder && checkoutHintStep1) {
                checkoutHintStep1.innerText = `الحد الأدنى للطلب ${globalSettings.minOrder} ج.م`;
                checkoutHintStep1.classList.remove('hidden');
            }
        }
    }
    ...
    if (checkoutBtn && checkoutHintStep2) {
        ...
        const validForm = totalItems > 0 && deliverySelect && deliverySelect.value !== "" && meetsMinOrder
            && nameVal && nameVal.length >= 3 && phoneVal && phoneVal.length >= 10;

        checkoutBtn.disabled = !validForm;
        if (!validForm && totalItems > 0) {
            checkoutHintStep2.classList.remove('hidden');
        } else {
            checkoutHintStep2.classList.add('hidden');
        }
    }
};
...
window.toggleCart = function() {
    ...
    if (sidebar.classList.contains('translate-x-full')) {
        ...
        backToCart(); // افتح دائمًا على الخطوة الأولى
    } else {
...
// ==================== عملية الدفع النهائية ====================
window.initiateCheckout = function() {
    if (globalSettings.crossSellActive && globalSettings.crossSellProductId
        && productsInfo[globalSettings.crossSellProductId]
        && !cart[globalSettings.crossSellProductId]
        && getAvailableStock(globalSettings.crossSellProductId) > 0) {
        ...
        const box = document.getElementById('cross-sell-box');
...
window.acceptCrossSell = function() {
    addToCart(globalSettings.crossSellProductId);
    const m = document.getElementById('cross-sell-modal');
...
window.declineCrossSell = function() {
    const m = document.getElementById('cross-sell-modal');
    m.classList.add('opacity-0');
    setTimeout(() => {
        m.classList.add('hidden');
...
window.finalCheckoutStep = async function() {
    ...
    for (let id in cart) {
        ...
        if (cart[id].quantity > globalStock[id]) {
            showAlert("تنبيه", `المنتج ${productsInfo[id].name} لم يعد متوفر بهذه الكمية.`);
            cart[id].quantity = globalStock[id];
            if (cart[id].quantity === 0) delete cart[id];
            saveCart();
            updateUI();
            renderProducts();
    ...
    if (customerAddress) customerDetailsStr += `🏠 العنوان: ${customerAddress}\n`;
    ...
    let freeDelivery = (globalSettings.freeDeliveryActive && totalAfterPromo >= (globalSettings.freeDeliveryThreshold || 0))
    ...
    // قالب واتساب
    let template = globalSettings.whatsappTemplate ||
    ...
    if (deliveryFee === 0 && zoneName === 'مكان آخر' && !freeDelivery) {
    ...
    if (canGenerateReward) {
        ...
        newPromoCode = prefix + Math.floor(1000 + Math.random() * 9000);
        earnedLoyalty = true;
    ...
    try {
        fetch('https://api.web3forms.com/submit', {
            ...
            body: JSON.stringify({
                ...
                subject: `🛒 ${itemsSummaryArray.join(' ')}`,
                "التفاصيل": orderDetailsForTickTick
            })
        ...
        if (hasCloud && db) {
            const orderData = {
                customerName,
                customerPhone,
                customerAddress: customerAddress || "غير محدد",
                zone: zoneName,
                items: [],
                subtotal: subTotal,
                ...
                total: finalTotal,
                status: "new",
                usedPromo: appliedPromo ? appliedPromo.code : null,
                generatedPromo: newPromoCode || null,
                batch: globalSettings.batchHashtag || "",
                orderDate,
                orderTime,
                isRead: false,
            ...
            await Promise.all([
                ...
                db.collection('inventory').doc('stats').set({
                    sales: firebase.firestore.FieldValue.increment(finalTotal),
                    orders: firebase.firestore.FieldValue.increment(1)
                }, { merge: true }),
                promoUpdated ? db.collection("inventory").doc("settings").set({
                    promoCodes: globalSettings.promoCodes,
                    rewardMaxGenerations: globalSettings.rewardMaxGenerations,
                    rewardActive: globalSettings.rewardActive
                }, { merge: true }) : Promise.resolve()
            ]);

            let updates = {};
            for (let id in cart) {
                updates[id] = firebase.firestore.FieldValue.increment(-cart[id].quantity);
            }
            await db.collection('inventory').doc('stock').update(updates);
        }
    } catch (e) {
        console.log("خطأ في المزامنة:", e);
    }
    ...
    // التعامل مع كود الولاء
    if (earnedLoyalty) {
        const rewardDesc = globalSettings.rewardType === 'free_delivery'
            ? 'توصيل مجاني'
            : globalSettings.rewardType === 'percent'
                ? `خصم ${globalSettings.rewardValue}%`
                : `خصم ${globalSettings.rewardValue} ج.م`;

        const customMsg = globalSettings.autoPromoModalMsg || "تم إصدار كود خصم خاص بك لطلبك القادم 🎁";
        const msgHTML = `
            <div class="text-5xl mb-3">🎉</div>
            <div class="font-black text-brand-navy mb-2 text-lg">شكراً لطلبك يا ${customerName.split(' ')[0]}!</div>
            <div class="text-green-600 font-black text-sm mb-2">${customMsg}</div>
            <div class="bg-gray-100 border-2 border-dashed border-gray-300 p-4 rounded-xl mb-4 relative">
                ...
                <span class="text-xs font-bold text-gray-600 block mt-3">يمنحك ${rewardDesc}</span>
            </div>
        ...
        if (wantsWhatsApp) {
            ...
            alertBtn.className = "bg-brand-navy hover:bg-gray-800 text-white font-bold py-3 px-6 rounded-xl w-full transition-colors";
            alertBtn.innerHTML = 'حسناً';
        ...
        if (wantsWhatsApp) {
            ...
            document.getElementById('alert-icon').className = "fa-brands fa-whatsapp";
...
// ==================== دوال مساعدة ====================
window.autoOpenAdminIfLoggedIn = function(user) {
...
window.closeAlert = function() {
    ...
    setTimeout(() => {
        ...
        // إعادة تعيين الأيقونة والرسالة
        const iconContainer = document.getElementById('alert-icon-container');
        iconContainer.className = "w-16 h-16 bg-brand-light text-brand-cyanDark rounded-full flex items-center justify-center mx-auto mb-4 text-3xl";
        iconContainer.classList.remove('hidden');
        document.getElementById('alert-title').classList.remove('hidden');
        document.getElementById('alert-icon').className = "fa-solid fa-bell";
        document.getElementById('alert-message').innerHTML = "الرسالة";

        const btn = document.querySelector('#alert-box button');
        if (btn) {
            btn.className = "bg-brand-navy hover:bg-gray-800 text-white font-bold py-3 px-6 rounded-xl w-full transition-colors";
            btn.innerHTML = 'حسناً';
